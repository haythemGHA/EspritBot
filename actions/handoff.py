...
class ActionHandoffOptions(Action):
    def name(self) -> Text:
        return "action_handoff_options-fr"
    ...
    async def run(
        ...
        if not any(
            ...
            dispatcher.utter_message(template="utter_no_handoff-fr")
        else:
            buttons = [
                {
                    "title": config.get("title"),
            ...
            dispatcher.utter_message(
                text="Je ne peux pas te transférer à un humain, \
                      mais je peux vous transférer vers l'un de ces bots",
                buttons=buttons,
            )
        return []
...
class ActionHandoff(Action):
    ...
    async def run(
        ...
        dispatcher.utter_message(template="utter_handoff-fr")
        handoff_to = tracker.get_slot("handoff_to")
